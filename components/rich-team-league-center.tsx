...
export function RichTeamLeagueCenter({ hub }: { hub: ProLeagueHub }) {
  ...
  return <main><LapHeader activeSport={hub.config.sportId} compact /><div className={`shell ${styles.page}`}>
    ...
    <section className={styles.section}><header className={styles.sectionHead}><div><span>Guia rápido</span><h2>Como acompanhar {hub.config.label}</h2><p>Comece pela classificação para entender a temporada, abra os times para consultar elenco e campanha e use a agenda para encontrar os próximos jogos e resultados.</p></div><Link href={`/agenda?sport=${hub.config.sportId}`}>Abrir agenda</Link></header></section>
    {hub.teams.length ? <section className={styles.section}><header className={styles.sectionHead}><div><span>Times</span><h2>Explore todas as equipes</h2><p>Clique para abrir elenco, idade, posição, campanha, notícias e calendário.</p></div><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Buscar time" aria-label={`Buscar time na ${hub.config.label}`} /></header><div className={styles.teamGrid}>{filtered.map((team) => <Link key={team.id} href={`/times/${hub.config.id}/${team.id}`}><TeamLogo team={team} /><span><strong>{team.shortName}</strong>{team.location || team.abbreviation ? <small>{[team.location, team.abbreviation].filter(Boolean).join(" · ")}</small> : null}</span><b>→</b></Link>)}</div></section> : null}
