...
function useLiveFeed(includeWorldCup = false) {
  const [data, setData] = useState<LivePayload | null>(null);
  const [error, setError] = useState(false);
  useEffect(() => {
    let active = true;
    const suffix = includeWorldCup ? "?includeWorldCup=1" : "";
    const load = async () => {
      try {
        const response = await fetch(`/api/live${suffix}`, { cache: "no-store" });
        if (!response.ok) throw new Error("feed unavailable");
        if (active) { setData(await response.json() as LivePayload); setError(false); }
      } catch { if (active) setError(true); }
    };
    void load();
    const unsubscribeRefresh = subscribeToLiveRefresh(load, { intervalMs: includeWorldCup ? 300_000 : 60_000 });
    const source = "EventSource" in window ? new EventSource(`/api/live/stream${suffix}`) : null;
...
export function WorldCupHub() {
  const { data, error } = useLiveFeed(true);
  const [round, setRound] = useState("all");
  const events = useMemo(() => (data?.worldCup.events ?? []).filter((event) => event.state === "post"), [data]);
  ...
  const visible = events.filter((event) => round === "all" || (event.round || "Partidas") === round);
  const bracket = useMemo(() => rounds.map((name) => ({ name, events: events.filter((event) => (event.round || "Partidas") === name) })), [events, rounds]);

  return <main><LapHeader /><div className="shell page hub-page"><section className="hub-hero hub-hero--cup"><p>Especial pós-Copa</p><h1>Copa do Mundo 2026</h1><span>A Espanha conquistou o bicampeonato mundial ao vencer a Argentina por 1 a 0 na prorrogação da final, em 19 de julho de 2026.</span><div className="hub-hero__stats"><strong>Espanha<small>campeã</small></strong><strong>1–0<small>final x Argentina</small></strong><strong>19 jul<small>final de 2026</small></strong></div></section>
  <section className="cup-brazil"><div><p>Campeã mundial</p><h2>Espanha no topo do mundo</h2><span>Ferran Torres marcou o gol do título na prorrogação. A LAP mantém esta página como arquivo histórico do torneio.</span></div><div className="cup-brazil__games"><a href="https://www.fifa.com/pt/tournaments/mens/worldcup/canadamexicousa2026/articles/espanha-argentina-resumo-melhores-momentos-final" target="_blank" rel="noreferrer">Ver confirmação e resumo oficial na FIFA ↗</a></div></section>
  {brazil.length ? <section className="cup-brazil"><div><p>Seleção Brasileira</p><h2>Brasil na Copa</h2><span>Arquivo de resultados da Seleção no torneio.</span></div><div className="cup-brazil__games">{brazil.slice(0, 3).map((event) => <EventCard key={event.id} score={event} cup />)}</div></section> : null}
  {events.length ? <section className="hub-section"><header className="hub-section__heading"><div><p>Arquivo de resultados</p><h2>Jogos encerrados da Copa</h2></div><select value={round} onChange={(event) => setRound(event.target.value)} aria-label="Filtrar fase"><option value="all">Todas as fases</option>{rounds.map((item) => <option key={item} value={item}>{item}</option>)}</select></header><div className="full-schedule__grid">{visible.map((event) => <EventCard key={event.id} score={event} cup />)}</div></section> : <section className="hub-section"><header className="hub-section__heading"><div><p>Arquivo oficial</p><h2>Resultados históricos</h2></div></header><p>O feed histórico de partidas não está disponível neste recorte da LAP. O resultado da final e o título acima permanecem baseados na confirmação oficial da FIFA.</p><a href="https://www.fifa.com/pt/tournaments/mens/worldcup/canadamexicousa2026/articles/copa-mundo-2026-classificacao-geral-final" target="_blank" rel="noreferrer">Consultar classificação final oficial ↗</a></section>}
  {bracket.length ? <section className="hub-section"><header className="hub-section__heading"><div><p>Caminho da competição</p><h2>Chaveamento por fase</h2></div></header><div className="bracket-grid">{bracket.map((group) => <article key={group.name} className="bracket-column"><h3>{group.name}</h3>{group.events.map((event) => <Link key={event.id} href={`/jogos/${event.sportId}/${event.id}?torneio=copa-2026`}><span>{event.home.name} {canDisplayScore(event) && <strong>{displayScoreValue(event, "home")}</strong>}</span><span>{event.away.name} {canDisplayScore(event) && <strong>{displayScoreValue(event, "away")}</strong>}</span></Link>)}</article>)}</div></section> : null}
  {error && <p className="status-note">O arquivo dinâmico da Copa não pôde ser atualizado agora; os dados históricos confirmados acima continuam válidos.</p>}</div></main>;
}
...
export function ScheduleHub() {
  ...
  const events = useMemo(() => {
    const all = data?.feeds.flatMap((feed) => feed.scores) ?? [];
    return Array.from(new Map(all.map((event) => [`${event.sportId}-${event.id}`, event])).values());
  }, [data]);
...
export function FavoritesHub() {
  ...
  const favoriteEvents = useMemo(() => {
    const events = data?.feeds.flatMap((feed) => feed.scores) ?? [];
  ...
  const favoriteSports = favorites.filter((item) => item.type === "sport" && PUBLIC_SPORTS.some((sport) => `sport:${sport.id}` === item.id));
