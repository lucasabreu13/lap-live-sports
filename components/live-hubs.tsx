...
export function WorldCupHub() {
  ...
  return <main><LapHeader /><div className="shell page hub-page"><section className="hub-hero hub-hero--cup"><p>Arquivo LAP</p><h1>Copa do Mundo 2026</h1><span>Resultados, partidas e histórico da competição encerrada em 19 de julho de 2026.</span><div className="hub-hero__stats"><strong>{ordered(events).live.length}<small>ao vivo</small></strong><strong>{ordered(events).next.length}<small>próximos</small></strong><strong>{ordered(events).finished.length}<small>encerrados</small></strong></div></section>
  <section className="cup-brazil"><div><p>Seleção Brasileira</p><h2>Brasil na Copa</h2><span>Arquivo de agenda e resultados da Seleção no torneio.</span></div><div className="cup-brazil__games">{brazil.length ? brazil.slice(0, 3).map((event) => <EventCard key={event.id} score={event} cup />) : <p>Os jogos da Seleção não estão disponíveis neste recorte.</p>}</div></section>
  ...
  <section className="hub-section"><header className="hub-section__heading"><div><p>Caminho da competição</p><h2>Chaveamento por fase</h2></div></header>{bracket.length ? <div className="bracket-grid">{bracket.map((group) => <article key={group.name} className="bracket-column"><h3>{group.name}</h3>{group.events.map((event) => <Link key={event.id} href={`/jogos/${event.sportId}/${event.id}?torneio=copa-2026`}><span>{event.home.name} {canDisplayScore(event) && <strong>{displayScoreValue(event, "home")}</strong>}</span><span>{event.away.name} {canDisplayScore(event) && <strong>{displayScoreValue(event, "away")}</strong>}</span></Link>)}</article>)}</div> : <div className="empty-card">O chaveamento não está disponível neste recorte.</div>}</section>
  {error && <p className="status-note">O arquivo da Copa não pôde ser atualizado agora.</p>}</div></main>;
}
